...
{
    static int16_t s[8] = {0, 0, 0, 0, 0, 0, 0, 0};
    static int16_t c[8] = {30 << 8, 30 << 8, 30 << 8, 30 << 8, 30 << 8, 30 << 8, 30 << 8, 30 << 8};
    const int16_t p[8] = {100, 101, 103, 107, 101, 102, 104, 108};
    ...
    while(1) {
        ...
        OCR0A = (s[0] + s[1] + s[2] + s[3] + (120 << 8)) / 256;
        OCR0B = (c[0] + c[1] + c[2] + c[3] + (120 << 8)) / 256;
        OCR2A = (s[4] + s[5] + s[6] + s[7] + (120 << 8)) / 256;
        OCR2B = (c[4] + c[5] + c[6] + c[7] + (120 << 8)) / 256;
    }
}
